...
bq --location=$GCP_LOCATION mk \
    ...
    "$GCP_PROJECT_ID:transcoder_jobs_$GCP_CUSTOMER_ID"

# Create a BigQuery Table to house job metadata from the Workflow
bq mk \
--table \
--label env:transcoding \
"$GCP_PROJECT_ID:transcoder_jobs_$GCP_CUSTOMER_ID.jobs" \
...
--table \
--label env:logs \
"$GCP_PROJECT_ID:transcoder_jobs_$GCP_CUSTOMER_ID.log-events-gke" \
...
--table \
--label env:logs \
"$GCP_PROJECT_ID:transcoder_jobs_$GCP_CUSTOMER_ID.log-events-batch" \
"../analytics/bq-log-events-gke-schema.json"
...
SERVICE_ACCOUNT_ID=$(bq show --format prettyjson --connection $GCP_PROJECT_ID.$GCP_LOCATION.bq-biglake-gcp-resources | jq --raw-output .cloudResource.serviceAccountId)
...
bq mkdef --connection_id=$GCP_PROJECT_ID.$GCP_LOCATION.bq-biglake-gcp-resources \
--noautodetect \
--object_metadata="SIMPLE" \
--metadata_cache_mode="MANUAL" \
gs://$GCP_GCS_SOURCE/* > def_input_file
...
--object_metadata="SIMPLE" \
--metadata_cache_mode="MANUAL" \
gs://$GCP_GCS_OUTPUT/* > def_output_file

bq mk --table \
  --external_table_definition=def_output_file \
  "transcoder_jobs_$GCP_CUSTOMER_ID.gcs-objects-output"
...
FROM `'transcoder_jobs_${GCP_CUSTOMER_ID}.jobs'` AS j
LEFT JOIN `'transcoder_jobs_${GCP_CUSTOMER_ID}.gcs-objects-input'` AS input
...
ON
    output.URI_PARTS[SAFE_OFFSET(2)] = "${GCP_GCS_OUTPUT}"
...
LEFT JOIN (
    ...
    FROM `'transcoder_jobs_${GCP_CUSTOMER_ID}.log-events-gke'`
    GROUP BY JobId
...
LEFT JOIN (
    SELECT
        JobId,
        ARRAY_AGG(STRUCT(LogName, Status, TimeStamp) IGNORE NULLS) AS Events
    FROM `'transcoder_jobs_${GCP_CUSTOMER_ID}.log-events-batch'`
    GROUP BY JobId
) AS batch_events USING(JobId)
GROUP BY
  ALL' \
transcoder_jobs_$GCP_CUSTOMER_ID.job-stats-summary
...
bq query \
--use_legacy_sql=false \
--target_dataset="transcoder_jobs_$GCP_CUSTOMER_ID" \
...
'INSERT INTO
    ...
    SELECT
        ...
        e.Timestamp
    FROM
        `'${GCP_PROJECT_ID}.transcoder_jobs_${GCP_CUSTOMER_ID}.jobs'` AS jobs
    INNER JOIN (
        SELECT
        ...
        FROM
            ...
            `'${GCP_PROJECT_ID}.transcoder_jobs_${GCP_CUSTOMER_ID}.events'`
        WHERE
    ...
    USING
        (JobId)
    LEFT JOIN
        `'${GCP_PROJECT_ID}.transcoder_jobs_${GCP_CUSTOMER_ID}.log-events-gke'` AS existing_e
    USING
        (JobId,
            LogName,
            Status,
...
'INSERT INTO
    ...
    INNER JOIN (
        ...
        FROM
            `'${GCP_PROJECT_ID}.transcoder_jobs_${GCP_CUSTOMER_ID}.workflows_googleapis_com_executions_system'`
        WHERE
            TIMESTAMP_TRUNC(timestamp, DAY) = TIMESTAMP(CURRENT_DATE())
        ...
        USING
            (JobId)
        LEFT JOIN
            `'${GCP_PROJECT_ID}.transcoder_jobs_${GCP_CUSTOMER_ID}.log-events-batch'` AS existing_e
        USING
            (JobId,
            LogName,
            Status,
...
gcloud logging sinks create bq-log-sink-gke-events\
 bigquery.googleapis.com/projects/${GCP_PROJECT_ID}/datasets/transcoder_jobs_${GCP_CUSTOMER_ID} \
  --log-filter='resource.type="k8s_cluster" AND resource.labels.cluster_name="'${GCP_GKE_CLUSTER_NAME}'" AND logName="projects/'${GCP_PROJECT_ID}'/logs/events"' \
...
gcloud logging sinks create bq-log-sink-workflow-events\
 bigquery.googleapis.com/projects/${GCP_PROJECT_ID}/datasets/transcoder_jobs_${GCP_CUSTOMER_ID} \
  --log-filter='logName="projects/'${GCP_PROJECT_ID}'/logs/workflows.googleapis.com%2Fexecutions_system"' \
  --use-partitioned-tables
...
gcloud logging sinks create bq-log-sink-batch-events\
 bigquery.googleapis.com/projects/${GCP_PROJECT_ID}/datasets/transcoder_jobs_${GCP_CUSTOMER_ID} \
  --log-filter='logName="projects/'${GCP_PROJECT_ID}'/logs/batch_task_logs" OR "projects/'${GCP_PROJECT_ID}'/logs/batch_agent_logs"' \
  --use-partitioned-tables
